...
- [ ] 2026-05-02: expose `openclaw` into the current user PATH during setup, keep OpenClaw running when DragonClaw exits from tray quit, reuse that existing service on the next launch, remove `DragonClaw` from the checking/initializing main title, keep `正在检查环境` on one line, and make startup checks degrade gracefully instead of hanging the onboarding flow.
