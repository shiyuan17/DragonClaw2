...
- `src/styles/tokens.css` is the implementation mirror of this document and should be treated as the app's single token source.
- New UI work must consume `--dc-*` tokens or semantic aliases that resolve to them; adding new hard-coded colors, radii, typography, spacing, or shadows outside the token layer is not allowed.
- Existing legacy variables such as `--bg-*`, `--text-*`, and `--accent-*` are compatibility shims only and should not be extended with new design values.
- If a new visual primitive is required, add it to the token layer first, then consume it from component or page styles.
